      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            {dummyQuestions.map(q => (
                <div key={q.id} onClick={() => setSelectedQuestion(q)} className={`p-6 rounded-lg cursor-pointer transition-all duration-300 ${selectedQuestion?.id === q.id ? 'bg-slate-700/80 ring-2 ring-cyan-500 shadow-[0_0_20px_rgba(34,211,238,0.2)]' : 'bg-slate-800/60 hover:bg-slate-700/50'}`}>
                    ...
                    <div className="mt-2 text-green-400 text-xs font-semibold">{q.status}</div>
                </div>
            ))}
        </div>
              ...
              <div className="p-3 bg-slate-900/50 rounded-md border border-slate-700 text-slate-100">
                {teamName}
                         ...
                         <div>
                            ...
                            <input type="number" id="bid-amount" value={bidAmount} onChange={(e) => setBidAmount(e.target.value)} placeholder="Enter your bid"  className="w-full p-3 bg-slate-900/50 rounded-md border border-slate-700 text-slate-100 placeholder:text-slate-500 focus:ring-cyan-500 focus:border-cyan-500"/>
                        </div>
                        <button type="submit" className="w-full bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-2 px-4 rounded transition-all duration-200">Place Bid</button>
                    </form>
                ) : (
